...
def technician_dashboard():
    ...
    cursor.execute("""
        SELECT 
            ...
            SUM(CASE WHEN t.status = 'In Progress' THEN 1 ELSE 0 END) as in_progress,
            SUM(CASE WHEN t.status = 'Resolved' THEN 1 ELSE 0 END) as resolved_today
        FROM assignments a
        JOIN tickets t ON a.ticket_id = t.id
        WHERE a.technician_id = ?
        AND a.is_active = TRUE
    ...
    for ticket in tickets:
        ...
        if ticket_dict['submitted_at']:
            ticket_dict['submitted_at_formatted'] = datetime.fromisoformat(ticket_dict['submitted_at']).strftime('%Y-%m-%d %H:%M')
        if ticket_dict.get('assigned_at'):
            ticket_dict['assigned_at_formatted'] = datetime.fromisoformat(ticket_dict['assigned_at']).strftime('%Y-%m-%d %H:%M')
        formatted_tickets.append(ticket_dict)
...
@login_required
def admin_dashboard():
    """Admin dashboard - system overview and manual assignment"""
    if session.get('role') != 'admin':
        return redirect(url_for('index'))
    ...
    # Flagged tickets (low confidence - need manual assignment)
    cursor.execute("""
    ...
    # Available technicians for manual assignment
    cursor.execute("""
        SELECT id, name, skills, current_workload, max_workload
        FROM technicians
        WHERE is_active = TRUE
...
def submit_ticket():
    """
    Submit new ticket with ML classification and confidence scoring
    Implements: Reliability requirement (60% threshold)
    """
    if session.get('role') != 'user':
        return jsonify({'error': 'Unauthorized'}), 403
    ...
    try:
        ...
        """, (
            ticket_number, subject, description, category, priority,
            session['user_id'], confidence_score, needs_manual_review,
            'Classified'  # Set to Classified after ML processing
        ))
        
        ticket_id = cursor.lastrowid
        
        # Update classified timestamp
        cursor.execute("""
            UPDATE tickets 
        ...
        log_system_action(
            'ticket_submit',
            'Ticket submitted and classified',
            f"Ticket: {ticket_number}, Category: {category}, Confidence: {confidence_score}%, Manual review: {needs_manual_review}",
            'user',
            session['user_id']
        )
        ...
        if assigned_technician:
            ...
            if needs_manual_review:
                response_data['message'] = 'Ticket flagged for manual review due to low confidence'
        ...
        conn.rollback()
        conn.close()
        log_system_action('ticket_submit', 'Ticket submission failed', str(e), 'user', session['user_id'], 'error')
        return jsonify({'error': 'Database error'}), 500
...
def update_ticket_status(ticket_id):
    ...
    try:
        ...
        if new_status == 'In Progress':
            cursor.execute("""
                UPDATE tickets
                SET in_progress_at = CURRENT_TIMESTAMP
                WHERE id = ? AND in_progress_at IS NULL
            """, (ticket_id,))
        ...
        log_system_action(
            'ticket_update',
            f'Ticket status updated to {new_status}',
            f"Ticket ID: {ticket_id}, Notes: {notes}",
            'technician',
            session['user_id']
        )
        ...
        if ticket_info:
            create_notification(
                ...
                f"Your ticket {ticket_info['ticket_number']} is now {new_status}"
            )
...
def manual_assign_ticket():
    ...
    if not ticket_id or not technician_id:
        return jsonify({'error': 'Ticket ID and Technician ID required'}), 400
    ...
    try:
        # Verify ticket exists and needs assignment
        cursor.execute("SELECT * FROM tickets WHERE id = ?", (ticket_id,))
        ticket = cursor.fetchone()
        ...
        log_system_action(
            'manual_assignment',
            'Ticket manually assigned',
            f"Ticket ID: {ticket_id}, Technician ID: {technician_id}, Reason: {reason}",
            'admin',
            session['user_id']
        )
        ...
        create_notification(
            ...
            ticket_id,
            'ticket_assigned',
            'New Ticket Manually Assigned',
            f"Admin assigned ticket {ticket['ticket_number']} to you"
        )
...
@login_required
def close_ticket(ticket_id):
    """Close a resolved ticket (admin only)"""
    if session.get('role') != 'admin':
        return jsonify({'error': 'Unauthorized'}), 403
...
if __name__ == '__main__':
    ...
    print("="*70)
    print("\nFeatures:")
    print("  ✓ Bcrypt password security")
    print("  ✓ ML classification with confidence scoring")
    print("  ✓ 60% confidence threshold for manual review")
    print("  ✓ Real-time Socket.IO notifications")
    print("  ✓ Complete ticket lifecycle (6 stages)")
    print("  ✓ Model versioning and logging")
    ...
    print("\n" + "="*70)
    print("SECURITY: All passwords are bcrypt-hashed")
    print("RELIABILITY: 60% confidence threshold enforced")
    print("REAL-TIME: Socket.IO notifications enabled")
    print("="*70)
